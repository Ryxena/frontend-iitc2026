...
import { toast } from "sonner"; // Pastikan Toaster dari Sonner sudah di-setup di Root Layout/Providers
...
export default function UploadProof({ status }: UploadProofProps) {
  ...
  // Kunci form upload jika status PENDING atau sudah SUCCESS/ACCEPTED
  const isLocked =
  ...
  const handleSubmit = () => {
    ...
    paymentMutation.mutate(formData, {
      onSuccess: () => {
        // Tampilkan toast bahwa bukti telah terkirim sesuai permintaan
        toast.success("Bukti pembayaran telah terkirim!");
        setUploadedFile(null);
      },
      onError: (error) => {
        ...
        <CardContent className="p-0">
          ...
          <div className="bg-slate-50/80 border-t border-slate-100 px-6 py-4 flex justify-end">
            <Button
              ...
              disabled={!uploadedFile || paymentMutation.isPending || isLocked}
