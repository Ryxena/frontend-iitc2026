...
import ForgotPasswordModal from "@/components/features/auth/ForgotPasswordModal"; // Import modal baru
import { loginSchema, type LoginInput } from "@/lib/schemas/auth.schema";
import { useLogin, getAuthErrorMessage } from "@/features/auth/hooks/use-login";

export default function LoginForm() {
  const [showPassword, setShowPassword] = useState(false);
  const [isForgotPasswordOpen, setIsForgotPasswordOpen] = useState(false); // State kontrol modal
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Lupa Password Link (Memicu Modal) */}
          <div className="flex justify-end pt-1">
            <button
              type="button"
              onClick={() => setIsForgotPasswordOpen(true)}
              className="text-sm font-medium text-[#2e2be3] hover:underline cursor-pointer"
            >
              Lupa Password?
            </button>
          </div>
